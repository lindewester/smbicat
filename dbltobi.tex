...
requires the presence of companions or conjoints to lift the part of the data given by tight morphisms to loose 1-cells. Before we discuss how this works, we briefly recall some definitions regarding transformations between functors of bicategories.
...
\begin{rmk}\label{thm:iso-strong}
...
Let $\mathcal{V}$ be a monoidal 2-category. A \textbf{$\mathcal{V}$-enriched bicategory} $\fB$ consists of the following data.
\begin{enumerate}
...
\item For each two objects $A,B$ of $\fB$, a natural isomorphisms 
  \begin{center}
    \input{lambdarho.tex}
    \end{center}
  \item The usual axioms for bicategories~\cite{maclane} hold.
 \end{enumerate}
\end{defn}
...
\begin{eg}
  ...
  \item Its 3-cells are \emph{cubical modifications}, defined below:
  \end{itemize}
\end{eg}

\begin{defn}[{\cite[Definition 13]{gg:ldstr-tricat}}]
...
Let $\mathcal{V}$ be a monoidal 2-category. Let ${\fB,\fC}$ be $\mathcal{V}$-enriched bicategories. A \textbf{$\mathcal{V}$-enriched functor} $F: {\fB} \rightarrow {\fC}$ consists of the following data:
\begin{enumerate}
...
There is a functor of locally cubical bicategories $\cL \maps \fDblf\to \fBicat$.
\end{thm}
\begin{proof}
