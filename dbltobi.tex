...
\fxnote{Should we recall the definition of icons somewhere?}
...
Howver up to equivalence it does not depend on these choices:
...
\begin{lem}\label{thm:h-locfr-uniq}
  ...
  \[\cH,\cH'\maps \cDbl(\lD,\lE)\too \cBicat_c(\cH(\lD),\cH(\lE)).\]
...
So far we have been able to deal with arbitrary tight transformations and their resulting colax transformations.
...
We are now ready to prove that $\cH$ lifts to a functor of locally cubical bicategories.
The definition of locally cubical bicategory can be found in~\cite{gg:ldstr-tricat}, but is also easy to write down: the 2-category $\cDbl$ has finite products, so we can write down the definition of bicategory in terms of hom-categories, with for instance composition functors $\bT(A,B) \times \bT(B,C) \to \bT(A,C)$, and then replace all the hom-categories with hom-double-categories.
(In other words, we pass through a general notion of bicategory enriched over a cartesian monoidal strict 2-category.)
\fxnote{Maybe we should formally define enriched bicategories in general?
Later on when we discuss products we do it at that level of generality.}
Thus, a locally cubical bicategory has objects, 1-morphisms, tight 2-morphisms, loose 2-cells, and 3-cells that fit in a square of tight and loose 2-cells.

Note also that a locally cubical bicategory with one object is precisely a monoidal double category in the sense of \cref{sec:symm-mono-double}, and thus the explicit description of the coherences in \cref{sec:symm-mono-double} can also be applied here.
...
Now, we can similarly obtain the notion of \emph{functor} between locally cubical bicategories by writing down the definition of functor between bicategories and replacing all categories by double categories:
