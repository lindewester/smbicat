...
from double categories to bicategories is given by a functor of a suitable sort.
...
It seems that we should have a functor from double categories with sufficient companions and conjoints
...
Let $\cDblf$
denote the sub-2-category of \cDbl\ containing the double
categories, all functors between them, and only transformations with loosely-strong companions; we regard it as a tricategory with only identity 3-cells.
...
\end{thm}
\begin{proof}
The first two requirements in \autoref{def:Iconfunc} are satisfied by Theorems \ref{thm:1-func} and \ref{thm:h-locfr}.
The third requirement amounts to the existence of a 2-cell $\phi\maps \behat * \alhat \iso \widehat{\be*\al}$ for every pair of transformations with loosely strong companions 
...
%
 \begin{equation}
  ...
  \end{equation}
commute. 
Here, we use the 'Godement product' $*$ of 2-cells in $\cDbl$ and $\cBicat$.  
...
For the icon  $\phi_u$ we can simply take the identity, since $\cH$ is strictly unital.
