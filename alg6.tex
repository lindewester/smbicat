...
\begin{defn}
We say that a monoidal double category $\mathbb{D}$ has {\bf local coequalizers} if for each object $a$, the hom-category $\mathbb{D}(a,a)$ has all coequalizers and the coequalizers are preserved by $\odot$. We write $\mathcal{D}bl^l$ for the 2-category of double categories with local coequalizers and $\cDblf^l$ for the 2-category of double categories with local coequalizers and companions.\fxnote[author=MS]{Previously we used $\cDblf$ for the 2-category of double categories with a restriction on the \emph{2-cells} to have loosely strong companions.  Does this match that?  If not, we should modify it.}
\end{defn}
...
An explicit description of the monoidal structure for the special case when ${\mathbb{D}}$ is a monoidal category was given in~\cite{westerthesis}. \fxnote[author=LW]{However, the choice of companions works only for monoidal categories, not for monoidal bicategories.}
...
This category has local coequalisers and loosely strong companions, and if \cat{C} has limits, it is monoidal~\cite[Examples 4.15, 9.2]{shulman:frbi}.
\end{proof}
