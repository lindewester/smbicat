...
\fxnote[author=MS]{This definition reads as if {\cat C} is a bicategory rather than a monoidal category, does it need fixing?}
\begin{itemize}
...
To obtain the result, we define the double category $\mathbb{A}lg({\cat C})$, whose horizontal bicategory corresponds to $\mathcal{A}lg({\cat C})$, and we show that $\mathbb{A}lg({\cat C})$ is fibrant and symmetric monoidal.
...
\begin{defn}
Let {\bf C} be a braided monoidal category equiped with a \fxnote*{We should define `dagger functor'}{dagger functor}. A Frobenius algebra in {\cat C} is a {\bf dagger Frobenius algebra} when the coalgebra is the dagger of the algebra. 
