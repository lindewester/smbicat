...
Any braided monoidal strong functor $F: {\bf C} \rightarrow {\bf D}$ which preserves coequalisers lifts to a braided monoidal strong double functor $\Alg{F}:  \Alg{\bf C} \rightarrow \Alg{ \bf D}$, respectively.
\end{prop}

\begin{proof}
The functor $\Alg(F)_0$ maps an algebra $(A, \tinymult[gray dot], \tinyunit[gray dot])$ to the algebra $(FA, F(\tinymult[gray dot])\circ \chi, F(\tinyunit[gray dot])\iota)$. It follows from functoriality that $F$ preserves algebra homomorphisms. The functor $\Alg(F)_1$ maps an A-B-bimodule ${\bf M}$ to $F({\bf M})\circ \chi_{A, M\otimes B} \circ (\id_A \otimes \chi_{M,B})$. On extended module homomorphisms, $\Alg(F)$ is defined componentwise: $\Alg(F)(f_l, f, f_r):=(Ff_l, Ff, Ff_r)$. This is well-defined by functoriality of $F$.
...
By the commuting diagram below, $F_{\odot}$ is a module homomorphism.
...
\node (m-1) at (-1,1.5) [] {$F(A\otimes M \otimes B \ten N \otimes C)$};
\node (m-3) at (-2,1) [] {$FA \ten FM \otimes FB \ten FN \ten FC$};
%leftlowersquare
...
\begin{thm}
Let $F:{\bf C} \rightarrow {\bf D}$ be a braided monoidal strong functor. The pseudofunctor of horizontal bicategories $\Alg (F): \Alg ({\bf C}) \rightarrow \Alg ({\bf D})$ is braided monoidal strong. When $F$ is sylleptic, or symmetric, then $\Alg (F)$ is sylleptic, or symmetric, respectively.
\end{thm}

\begin{proof}
Since $F$ is monoidal strong, the transformations $\chi$ and $\chi_u$ of $\mathbb{A}lg(F)$ are horizontally strong. The result follows from Proposition~\ref{prop:mfunctor} and Theorem~\ref{thm:lcbcfunctor}.
\end{proof}
...
\begin{prop}
Let ${\bf C, D}$ be braided monoidal dagger categories and let $F:{\bf C} \rightarrow {\bf D}$ be a braided monoidal strong dagger functor that preserves coequalisers. The functor $F$ lifts to a braided monoidal strong pseudofunctor $2(F): 2({\bf C}) \rightarrow 2({\bf D})$.
\end{prop}
