...
\begin{defn}\label{def:monfunc}
  Let $\D$, $\E$ be (braided/symmetric) monoidal double categories.  A {\bf (braided/symmetric) lax monoidal double functor} $F: \D \rightarrow \E$ is a pseudo double functor $F$, together with a vertical transformation $\phi : \otimes \circ (F,F) \rightarrow F \circ \otimes$ and also
  a 1-morphism $\phi_{u_0} : I_{\E} \rightarrow F(I_{\D})$ and a 2-morphism $\phi_{u_1} : U_{I_{\E}} \rightarrow F(U_{I_{\D}})$, which together form a vertical transformation $\phi_u$ from the functor $I_{\E}$ to the functor $F \circ I_{\D}$.
  % I don't understand the following.  To the extent that I understand it, it doesn't seem true.
  where the natural transformations ${I_{\E}}_{\odot}, {I_{\E}}_{U}, (F \circ I_{\D})_{\odot}$ and $ (F \circ I_{\D})_{U}$ are all identities.
  %
  These functors and structure maps satisfy the usual axioms for (braided/symmetric) monoidal functors with respect to $\otimes$.
\end{defn}
...
\begin{defn}\label{Def:monverttrans}
  ...
  Explicitly, this means that the following equality holds:
...
% Seems there should be a unit condition too.
...
We use Theorem ~\ref{thm:trifunctor2} to prove that the bicategory of algebras, bimodules and bimodule homomorphisms in a bicatehory ${\cat B}$ is monoidal , and symmetric or braided whenever this is true for ${\cat B}$. In addition we show that this result holds when we restrict to dagger Frobenius algebras, dagger bimodules and bimodule homomorphisms. As a result, the equivariant completion of a bicategory introduced in ~\cite{carquevillerunkel} preserves braided and monoidal structure, and the bicategory $CP({\bf C})$ of mixed quantum states introduced in ~\cite{heunenvicarywester} is symmetric monoidal.

The bicategory $\mathcal{A}lg({\cat B})$ is a new bicategory constructed from a bicategory ${\cat B}$ consisting of the following data:
...
This is the horizontal bicategory of the fibrant double category $\Alg({\cat B})$, which we define below. We will often refer to these categories as $\mathbb{A}lg$ and $\mathcal{A}lg$, when the choice of the original bicateory ${\bf B}$ is not important.
We use the language of string diagrams for braided monoidal categories, first introduced by M. Kelly and M.L. Laplaza in ~\cite{kellylaplaza}, and its extended graphical calculus for monoidal bicategories described in ~\cite{bms}. In this diagrammatic language, we depict 0-cells as coloured surfaces, with the convention to use white for the unit 0-cell. We draw 1-cells (and identity 2-cells) as lines, and 2-cells generally as boxes between the source 1-cell in the bottom and the target 1-cell on top. We write horizontal composition $(\hor)$ and vertical composition $(\ver)$ in the horizontal and vertical direction, respectively. The tensor product $(\tens)$ as juxtaposition in the third direction, which is pointing into the paper. In our pictures this will be expressed as overlapping surfaces, as illustrated below. The reader who is not familiar with this graphical calculus may ignore the coloured surfaces and view the lines and boxes as string diagrams. This corresponds to the situation where the monoidal bicategory has only one object, which makes it a braided monoidal category. 
...
\subsection{The double category $\Alg$}
...
\begin{calign}
  \begin{pic}[scale=1.5]
    \draw[regionA] (-.9,-.5) -- (-.9,1) -- (.9,1) -- (.9,-.5) -- (-.9,-.5);
    \draw (0,0.5) to (0,1)node[above] {$A$};
    \draw (.6,-.5) node[below]{$$} to[out=90,in=0] (0,0.5);
    \draw (0,0.5) to[out=180,in=90] (-.6,-.5)node[below] {$$};
    \node [white dot, node on layer=main] (m) at (0,.5) {};
    %%%%
    \draw[regionB] (-.7,-.7) -- (-.7,.8) -- (1.1,.8) -- (1.1,-.7) -- (-.7,-.7);
    \node[white dot] (m) at (0.2,.3) {};
    \draw (m.center) to (0.2,.8)node[above] {$B$};
    \draw (.8,-.7) node[below]{$B$} to[out=90,in=0] (m.center);
    \draw (m.center) to[out=180,in=90] (-.4,-.7)node[below] {$B$};
  \end{pic}
\end{calign}
...
\draw[fill=red, draw=black, opacity=0.25] (-1,0) -- (-1,-2) -- (0,-2) -- (0,0) -- (-1,0);  
\draw[fill=blue, draw=black, opacity=0.25] (0,0) -- (0,-2) -- (1,-2) -- (1,0) -- (0,0);      
      \node[morphism, minimum width=10mm] (l) at (0,-.8) {$\mathbf{M}$};
      \draw (0,0) to (l.north);
      \draw (l.south) to (0,-2) node[below] {$M$};
      \draw (l.-45) to[out=-90,in=90] (.6,-2) node[below] {$B$};
      \draw (l.-135) to[out=-90,in=90] (-.6,-2) node[below] {$A$};
      %%%%%%%%%%%%%%%%%%%%%%%
      \draw[fill=purple, draw=black, opacity=0.25] (-.75,-.3) -- (-.75,-2.3) -- (0.25,-2.3) -- (0.25,-.3) -- (-.75,-.3);  
\draw[fill=green, draw=black, opacity=0.25] (0.25,-.3) -- (0.25,-2.3) -- (1.25,-2.3) -- (1.25,-.3) -- (0.25,-.3);      
      \node[morphism, minimum width=10mm] (l) at (0.25,-1.1) {$\mathbf{N}$};
      \draw (0.25,-.3) to (l.north);
      \draw (l.south) to (0.25,-2.3) node[below] {$N$};
      \draw (l.-45) to[out=-90,in=90] (.85,-2.3) node[below] {$D$};
      \draw (l.-135) to[out=-90,in=90] (-.25,-2.3) node[below] {$C$};
    \end{pic}}
\end{equation}
...
\subsection{The braided monoidal structure of $\Alg$}
...
What is left to prove is that the diagrams given in points (iv),(v),(vi), and (ix) of Definition 2.9 of ~\cite{shulman} commute. Because $\fu$ and $\fx$ are globular, it suffices to verify commutativity of the diagrams only for the middle components of the extended module homomorphisms.
...
\subsection{Companions and conjoints in $\Alg$}
...
\begin{eg}
When we take the category $\mathcal{A}B$ of Abelian groups and group homomorphisms as our symmetric monoidal category, we obtain the double category $\cMod$ of rings, ring homomorphisms, bimodules and bimodule homomorphisms. As a consequence of Lemma's ~\ref{lem:algdouble}, ~\ref{lem:algsymmon}, and ~\ref{lem:algfib}, and Lemma 5.6 of ~\cite{shulman}, the horizontal bicategory $\cMod$ of $\lMod$ is a symmetric monoidal.
\end{eg}

\begin{thm}\label{thm:eqcomp}
Let {\bf B} be a bicategory. The equivariant completion of {\cat B} is monoidal whenever {\bf B} is monoidal; it is braided whenever  {\bf B} is braided; and it is symmetric whenever {\bf B} is symmetric.
\end{thm}

\begin{proof}
This follows directly from Lemmas \ref{lem:algdouble}, \ref{lem:algsymmon}, \ref{lem:algfib}, and from Theorem 5.1 of ~\cite{shulman}.
\end{proof}

 \begin{cor}\label{cor:symmonbicat}
Let {\bf C} be a braided monoidal category. The bicategory $\mathcal{A}lg$ of algebras, bimodules, and bimodule homomorphisms in {\cat C} is  monoidal. 
\end{cor}

\begin{proof}
It follows from Theorem \ref{thm:eqcomp} and the fact we can regard a braided monoidal category as a monoidal bicategory with one object, that $\mathcal{A}lg$ is monoidal.
\end{proof}
...
\begin{prop}
Let {\bf B} be a monoidal bicategory with one object. The bicategory $\mathcal{A}lg$ of algebras, bimodules, and bimodule homomorphisms in {\cat B} is braided monoidal. When ${\bf B}$ is braided, then $\mathcal{A}lg$ is symmetric.
\end{prop}
...
\subsection{Commutative special dagger Frobenius algebras}

We will demonstrate that the statement still holds when we restrict to the subbicategory of (commutative/special/dagger) Frobenius algebras, (dagger) bimodules, and bimodule homomorphisms. 
...
\begin{prop}
Let ${\cat C}$ be a symmetric monoidal dagger category. The bicategory of dagger Frobenius algebras, dagger bimodules and bimodule homomorphisms of ${\cat C}$ is symmetric monoidal.
\end{prop}

\begin{proof}
We take the category of dagger Frobenius algebras and self conjugate algebra homomorphisms of {\cat C} as $\D_0$. For $\D_1$ we take the category of dagger bimodules and extended bimodule homomorphisms $(f_1,f,f_2)$ {\cat C}, of which $f_1$ and $f_2$ are self conjugate. This ensures that ~\ref{eq:companion} is a dagger bimodule.  These categories are symmetric monoidal subcategories of our original $\D_0$ and $\D_1$, since self-conjugate morphisms are closed under the tensor product and all structure isomorphisms of the monoidal categories are self conjugate. All other steps of the construction hold in the same way as in the general case.  
\end{proof}
