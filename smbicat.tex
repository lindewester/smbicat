...
requires the presence of companions or conjoints.  We briefly recall the definition of transformations of bifunctors. If $F,G\maps \cA\to\cB$ are functors between bicategories, then an
...
An {\bf algebra homomorphism} $f: (A, \tinymult[white dot], \tinyunit[white dot]) \rightarrow (B, \tinymult[gray dot], \tinyunit[gray dot])$ is a morphism $f:A \rightarrow B$ in ${\bf C}$ that respects the multiplication, $f \circ \tinymult[white dot] = \tinymult[gray dot] \circ (f \otimes f)$, and the unit, $\tinyunit[gray dot] = f \circ \tinyunit[white dot]$. Algebras and algebra homomorphisms form a symmetric monoidal category with the usual tensorproduct for morphisms, the usual structure natural isomorphisms, and for objects, the tensor product $(A, \tinymult[white dot], \tinyunit[white dot]) \otimes (B, \tinymult[gray dot], \tinyunit[gray dot]) := (A \otimes B, \prodmult{white dot}{gray dot}, \whiteunit \grayunit)$. The unit is the trivial algebra $(I, \lambda_I, \mathid_I)$. 
...
By commutativily of the left square, $c \circ (f \otimes g)$ is another coequaliser of the upper two parallel maps, hence $f \tinydot[gray dot] g$ exists and is unique. 
...
What is left to prove is that that diagrams of \ref{eq:braid1}-\ref{eq:mon1} of Definition ~\ref{def:symmondoub} commute. Because $\upsilon$ and $\xi$ are globular, it suffices to verify commutativity of the diagrams only for the middle components of the extended module homomorphisms.

The first equation of \ref{eq:mon1} corresponds to the front square of the diagram below. The unlabeled arrows represent the coequalisers defining their target object.
