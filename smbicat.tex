...
\begin{rmk}
  ...
  below).  They also don't consider symmetries or the functoriality of
  the construction.
\end{rmk}
...
\begin{thm}\label{thm:mondbl-monbi-intro}
  ...
  or symmetric, so is $\cH(\lD)$. Furthermore, the assignment $\cH$ extends to a trifunctor, so we can use it to construct functors and transformations between monoidal bicategories as well.
\end{thm}
...
1x2-categories --- or even 1x1x1-categories, i.e.\ triple categories, as in~\cite{gp:intercategories-i,gp:intercategories-ii}.
Any $(n\times k)$-category which satisfies a suitable
...
show that it preserves monoidal, braided, and symmetric structures. Finally, in \S\ref{sec:Alg} we illustrate our method by constructing the symmetric monoidal bicategory ${clA}lg$ of algebras, bimodules and bimodule homomorphisms.
...
We will use the language of string diagrams for symmetric monoidal categories, first introduced by M. Kelly and M.L. Laplaza in ~\cite{kellylaplaza}. A good overview is given in ~\cite{selinger}. In the string diagrammatic language, we depict objects (and identity morphisms) as lines $\tinyid$, morphisms generally as boxes with the source object in the bottom and the target object on top. However, multiplication maps are depicted as circles $\tinymult[white dot]$, and the swap map as crossing lines; we write tensor products as a juxtaposition of lines/boxes. This diagrammatic language is sound and complete for symmetric monoidal categories, as shown in ~\cite{selinger}.

We will consider the abstract notion of an {\bf algebra} $(A, \tinymult[white dot], \tinyunit[white dot])$. This is an object, $A$, of a symmetric monoidal category, ${\bf C}$, together with a multiplication map, $\tinymult[white dot]: A \times A \rightarrow A$, and a unit map, $\tinyunit[white dot]: I \rightarrow A$, satisfying the associativity and unit law below.
...
An {\bf algebra homomorphism} $f: (A, \tinymult[white dot], \tinyunit[white dot]) \rightarrow (B, \tinymult[gray dot], \tinyunit[gray dot])$ is a morphism $f:A \rightarrow B$ in ${\bf C}$ that respects the multiplication, $f \circ \tinymult[white dot] = \tinymult[gray dot] \circ (f \otimes f)$, and the unit, $\tinyunit[gray dot] = f \circ \tinyunit[white dot]$. Algebras and algebra homomorphisms form a symmetric monoidal category with the usual tensor product for morphisms, the usual structure natural isomorphisms, and for objects, the tensor product $(A, \tinymult[white dot], \tinyunit[white dot]) \otimes (B, \tinymult[gray dot], \tinyunit[gray dot]) := (A \otimes B, \prodmult{white dot}{gray dot}, \whiteunit \grayunit)$. The unit is the trivial algebra $(I, \lambda_I, \mathid_I)$. 
An A,B-{\bf bimodule} on an object $M$ is a morphism ${\bf M}: A \otimes M \otimes B \rightarrow M$, satisfying the composition and unit law below.
...
\begin{equation}\label{eq:bimodule}
    ...
    \end{pic}
  \end{equation}
For readability, we sometimes write A-B-bimodules ${\bf M}$ as $_A{\bf M}_B$ and we abbreviate the following compositions: ${}_{\tinydot[white dot]}{\bf M} := {\bf M} \circ (\tinyunit[white dot] \otimes \mathid)$, ${\bf M}_{\tinydot[gray dot]}:= {\bf M} \circ (\mathid \otimes \tinyunit[gray dot])$, and $_{\tinydot[white dot]}{\bf M}_{\tinydot[gray dot]}:= {\bf M} \circ (\tinyunit[white dot] \otimes \tinyunit[gray dot])$. An {\bf extended module homomorphism} ${\bf f}: {\bf M} \rightarrow {\bf N}$ is a triple of morphisms $(A \xrightarrow{f_l} C, M \xrightarrow{f} N, B \xrightarrow{f_r} D)$ such that $f \circ M = N \circ (f_l \otimes f \otimes f_r)$. When $f_l$ and $f_r$ are both identities, this corresponds to the standard definition of a {\bf bimodule homomorphism}.
Bimodules and extended bimodule homomorphisms form a symmetric monoidal category, where the tensor product is defined on morphisms component wise. The unit object and the tensor product for objects are defined below:
\begin{equation}
{\bf I}:=
\smash{{\hspace{-5pt}\ensuremath{\begin{pic}[scale=1.2,string,yscale=-1]
      \node (0) at (0,0) {};
...
\end{pic}
\end{equation}
By the notation for the unit object, we mean that we apply the multiplication map, $\tinymult[gray dot]$, on $I$ twice. We draw it symmetrically because of the associativity condition.
The associator, unitor, and swap natural isomorphisms are given by the maps with the following components: 
...
\begin{equation}\label{eq:coequaliser}
  \begin{pic}[xscale=4,yscale=1.66, thin]
    ...
    \draw[->] ([yshift=1.5pt]tl.east) to node[above] {${}_{\tinydot[white dot]}\mathbf{M} \otimes \mathid_{N}$} ([yshift=1.5pt]t.west);
    \draw[->] ([yshift=-1.5pt]tl.east) to node[below] {$\mathid{M} \otimes \mathbf{N}_{\tinydot[black dot]}$} ([yshift=-1.5pt]t.west);
    \draw[->] ([yshift=1.5pt]bl.east) to node[above] {${}_{\tinydot[white dot]}\mathbf{M'} \otimes \mathid{N'}$} ([yshift=1.5pt]b.west);
    \draw[->] ([yshift=-1.5pt]bl.east) to node[below] {$\mathid{M'} \otimes \mathbf{N'}_{\tinydot[black dot]}$} ([yshift=-1.5pt]b.west);
  \end{pic}
 \end{equation}

By commutativity of the left square, $c \circ (f \otimes g)$ is another coequaliser of the upper two parallel maps, hence $f \tinydot[gray dot] g$ exists and is unique. 
  The module ${\bf M} \tinydot[gray dot] {\bf N}$ is given by the map that makes the diagram below commute. It exists and is unique by uniqueness of coequalisers, taking commutativity of the left square into account, and the fact that $\otimes$ preserves coequalisers. 
...
Note that ${\bf N}_{\tinydot[gray dot]} \otimes {}_{\tinydot[gray dot]}{\bf N}$ is a module. It is easy to check that ${\bf M}_{\tinydot[gray dot]}{\bf N}$ satisfies the module axioms as well, using commutativity of the diagram above and the fact that $c$ is epic. Pasting the two diagrams together into a cube proves that $(f \otimes g)$ is a bimodule homomorphism.
 
It is left to verify functoriality of $\ominus$. When we unfold the definitions we see that we only need to check that $(f\tinydot[gray dot]f') \circ( g \tinydot[gray dot] g') = (f \circ f')\tinydot[gray dot] (g \circ g')$, and $\mathid_{\bf M} \tinydot[gray dot] \mathid_{\bf M'} = \mathid_{\bf M \tinydot[gray dot] M'}$. The two sides of the first equality are the unique maps making the two diagram below commute. If we forget the middle part of the left diagram, the diagrams are equal by the exchange law for $\otimes$ and $\circ$. As a consequence, the dashed arrows must me equal too. A similar argument shows that $\mathid_{\bf M} \tinydot[gray dot] \mathid_{\bf M'} = \mathid_{\bf M \tinydot[gray dot] M'}$.
 
 \begin{equation}
  \begin{pic}[xscale=2.3, yscale=1.5, thin]
    \node (tl) at (-.3,1) {$M \otimes B \otimes N$};
    \node (bl) at (-.3,0) {$L \otimes C \otimes K$};
    \node (ld) at (-.3,-1) {$P \otimes D \otimes Q$};
    \node (t) at (1,1) {$M \otimes N$};
    \node (b) at (1,0) {$L \otimes K$};
    \node (md) at (1,-1) {$P \otimes Q$};
    \node (tr) at (1.7,1) {${M}_{\tinydot[gray dot]} {N}$};
    \node (br) at (1.7,0) {${K}_{\tinydot[gray dot]} {L}$};
    \node (rd) at (1.7,-1) {${M}_{\tinydot[gray dot]} {N}$};
    \draw (tl) to  (-.3,.65);
    \node at (-.3, 0.5) {$f \otimes i \otimes g$};
    \draw[->] (-.3,.35) to  (bl);
    \draw[->] (t) to node[right] {$f \otimes g$} (b);
    ...
    \draw[->] ([yshift=1.5pt]tl.east) to node[above] {${}_{\tinydot[white dot]}\mathbf{M} \otimes \mathid_{N}$} ([yshift=1.5pt]t.west);
    \draw[->] ([yshift=-1.5pt]tl.east) to node[below] {$\mathid_{M} \otimes \mathbf{N}_{\tinydot[black dot]}$} ([yshift=-1.5pt]t.west);
    \draw[->] ([yshift=1.5pt]bl.east) to node[above] {${}_{\tinydot[white dot]}\mathbf{K} \otimes \mathid_{L}$} ([yshift=1.5pt]b.west);
    \draw[->] ([yshift=-1.5pt]bl.east) to node[below] {$\mathid_{K} \otimes \mathbf{L}_{\tinydot[black dot]}$} ([yshift=-1.5pt]b.west);
    \draw (bl) to  (-.3,-.35);
    \node at (-.3,-.5) {$h \otimes j \otimes k$};
    \draw[->] (-.3,-.65) to  (ld);
    \draw[->] (b) to node[right] {$h \otimes k$} (md);
   % \draw[->, dashed] (tr) to node[right] {$\rho_M \tinydot[gray dot] \rho_N$} (rd);
    \draw[->] (md) to node[above] {$c_D$} (rd);
    \draw[->] ([yshift=1.5pt]ld.east) to node[above] {${}_{\tinydot[white dot]}\mathbf{P} \otimes \mathid_{Q}$} ([yshift=1.5pt]md.west);
    \draw[->] ([yshift=-1.5pt]ld.east) to node[below] {$\mathid_{P}  \otimes \mathbf{Q}_{\tinydot[black dot]}$} ([yshift=-1.5pt]md.west);
%    \draw[->, dashed] (br) to [in=90, out=90] {$\hat{\xi}^{-1}$} (rd);
  \end{pic}  
   \begin{pic}[xscale=2.3, yscale=1.5, thin]
    ...
    \draw (tl) to  (-.3,0.25);
    \draw[->] (-.3,-0.25) to  (ld);
    \node at (-.3,0.125) {$(k \circ f) \otimes (j \circ i)$};
    \node at (-.3,-.125) {$\otimes (k \circ g)$};
    \draw (t) to  (1,0.3);
    \draw[->] (1,0.1) to  (md);
    \node at (1,0.2) {$(h \circ f) \otimes (k \circ g)$};
    \draw[dashed] (tr) to (1.7,-0.1);
    \node at (1.7,-.2) {$(h \circ f)_{\tinydot[gray dot]}(k \circ g)$};
...
The equations below express the fact that $\pi_1$ is a cocone of $_{\tinydot [gray dot]}{\bf M_1} \otimes {}_{\tinydot[gray dot]}{\bf M_2} \otimes \mathid_{M_3}$ and $\mathid_{M_1} \otimes {\bf M_2}_{\tinydot[gray dot]} \otimes  {\bf M_3}_{\tinydot[gray dot]}$.
...
The last step for showing that $\pi_1$ is a coequaliser, is to prove that if we have another cocone $f$, then $f$ factorises through $\pi_1$. Plugging in the unit for $B$ gives us that $f \circ (_{\tinydot [gray dot]}{\bf M_1}_{\tinydot[gray dot]} \otimes {}_{\tinydot[gray dot]}{\bf M_2} \otimes \mathid_{M_3}) = f( \mathid_{M_1} \otimes {}_{\tinydot[gray dot]}{\bf M_2}_{\tinydot[gray dot]} \otimes  {\bf M_3}_{\tinydot[gray dot]})$, so by the unit law for modules, $f$ is a cocone of $\mathid_{M_1} \otimes {}_{\tinydot[gray dot]}{\bf M_2}\otimes \mathid_{M_3}$ and $\mathid_{M_1} \otimes \mathid_{M_2}\otimes {\bf M_3}_{\tinydot[gray dot]}$. Similarly, $f$ is a cocone of $_{\tinydot[gray dot]}{\bf M_1} \otimes \mathid_{M_2} \otimes \mathid_{M_3}$ and $\mathid_{M_1} \otimes {\bf M_2}_{\tinydot[gray dot]} \otimes \mathid_{M_3}$.   
...
The first statement implies that $f$ factorises through $c_C$; therefore, there exists a morphism $f'$, such that $f = f' \circ c_C$. From the second statement we deduce that $f'$ factorises through $\tilde{c}_B$, which finishes our proof. This follows from the equations below, and the fact that $\mathid \otimes c_C$ is epic, since $\otimes$ preserves coequalisers. 
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
The first equality is obtained by interchanging $c_C$ and ${\bf M_1} \otimes \mathid$, the second holds because $f$ is a cocone of $_{\tinydot [gray dot]}{\bf M_1} \otimes \mathid_{M_2} \otimes \mathid_{M_3}$ and $\mathid_{M_1} \otimes { \bf M_2}_{\tinydot [gray dot]} \otimes \mathid_{M_3}$, the third since $f= f' \circ (\mathid \otimes c_C)$, and the fourth by definition of ${\bf M_2}_{\tinydot[gray dot]}{\bf M_3}$. This means that there exists a morphism $f''$ such that $f= f' \circ (\mathid \otimes c_C) = f'' \circ \tilde{c}_B \circ (\mathid \otimes c_C)$, hence $f$ factorizes through $\pi_1$. A similar argument shows that $\pi_2$ is a coequaliser as well. 

The components of the {\bf left unitor} $l_{\bf M}: U_A \ominus {\bf M} \rightarrow {\bf M}$ are given by $(\mathid, l_{M}, \mathid)$, where $ l_{M}$ is defined as follows: By the module laws, ${\bf M}_{\tinydot[gray dot]}: A \otimes M \rightarrow M$ is a coequaliser of the maps $_{\tinydot[gray dot]}{\bf A} \otimes \mathid_{M}$ and $\mathid_{A} \otimes {\bf M}_{\tinydot[gray dot]}$. The canonical isomorphisms between ${\bf M}$ and ${\bf A}_{\tinydot[gray dot]}{\bf M}$ is the middle component of $l_{\bf M}$, and naturality of $l$ follows from commutativity of the diagram below. We can define $r$, and prove its naturality, in a similar way.
...
Unfolding definitions shows that the globular isomorphism $\upsilon$ from $U \circ (\otimes)$ to $\otimes \circ (U \times U)$ is the identity. We define the exchange law, ${\bf \xi}$, between the functors $\ominus$ and $\otimes$ as $(\mathid, \hat{\xi}, \mathid)$, where $\hat{\xi}$ is the unique morphism completing the commuting diagram below.
In this diagram, $c$, $c_M$, and $c_N$ are the coequalisers defining  $({M_1} \otimes {N_1})_{\tinydot[gray dot]}({M_2} \otimes {N_2})$, ${M_1}_{\tinydot[gray dot]} M_2$, and ${N_1}_{\tinydot[gray dot]} N_2$, respectively. By a similar argument given while defining $a$, one can show that $c_M \otimes c_N$ is the coequaliser of the lower two parallel maps in the diagram. The two leftmost vertical morphisms are natural isomorphisms swapping the objects. We will call the middle morphism $\hat{\sigma}$.
...
\begin{equation}\label{def:xi}
  \begin{tikzpicture}[xscale=5, yscale=2, thin]
    ...
    \draw[->, dashed] (tr) to node[right] {$\iso \hat{\xi} $} (br);
    \draw[->] (t) to node[above] {c} (tr);
...
What is left to prove is that the diagrams given in points \ref{eq:mon1}, \ref{eq:mon2}, \ref{eq:mon3}, and \ref{eq:braid1} in Definition ~\ref{def:symmondoub} commute. Because $\upsilon$ and $\xi$ are globular, it suffices to verify commutativity of the diagrams only for the middle components of the extended module homomorphisms.

The first equation of \ref{eq:mon1} corresponds to the front square of the diagram below. The unlabelled arrows represent the coequalisers defining their target object.
The bottom square and the upper right square commute by the definition of $\hat{\xi}$. The square at the back commutes by coherence of ${\cat C}$. The upper left and lower right square commute by definition of $a$. The lower left and the top square commute by definition of $\xi$ and $\tinydot[gray dot]$.
...
\draw[->] (m4) to node[left] {$a \otimes a$} (b4);
%uppersquare
...
\draw[->] (u1) to node[above] {$\mathid \otimes \hat{\sigma}$} (u2);
\draw[->] (u1) to node[below] {} (u3); 
\draw[->, cross] (u2) to node[above] {} (u4);
\end{tikzpicture}
\end{equation}

All other equations concerning $\xi$ are proven in the same way. These are the second and third equation of \ref{eq:mon1}, the first equation of \ref{eq:mon2}, the first and the third equation of \ref{eq:mon3}, and the first equation of \ref{eq:braid1}.
...
These remaining equations are the second equation of \ref{eq:mon2}, the second and the fourth equation of \ref{eq:mon3}, and the second equation of \ref{eq:braid1}.
\end{proof}

Finally, we will prove that $\Alg(\cat C$) is fibrant. this is the last step we need to take to show that we can lift the symmetric monoidal structure of $\Alg({\cat C})$ to the bicategory ${\cA}lg({\cat C})$. 

\begin{prop}\label{lem:algfib}
The double category $\Alg({\cat C})$ is fibrant.
\end{prop}
...
\draw[->] ([yshift=-1.5pt] bl.east) to node[below] {$\mathid_{B} \otimes {U_B}_{\tinydot[gray dot]}$} ([yshift=-1.5pt] bm.west);
 ...
 \draw[->] (tm.east) to node[below] {$\hat{f}_{\tinydot[gray dot]}$} (r2);
  \draw[->] (bm.east) to node[above,xshift=-4pt] {$c_B$} (r3.west);
...
It is easy to check that $\hat{f}_{\tinydot[gray dot]}$ and $_{\tinydot[white dot]}{\hat{f}}$ are coequalisers. The identity morphism on $B$ makes the front part of the diagram commute, which means that it is the unique map that makes this diagram commute. If we assume the double category to be strict, it follows that $f \tinydot[gray dot] \mathid_B = \mathid_B$, and thus $\eta_{\hat{f}} \ominus \epsilon_{\hat{f}} = 1_{\hat{f}}$.
...
\end{proof}
 ...
 \begin{thm}
Let {\bf C} be a symmetric monoidal bicategory. The bicategory $\mathcal{A}lg$ of algebras, bimodules, and bimodule homomorphisms in {\cat C} is symmetric monoidal. 
\end{thm}
...
The sub bicategory of (commutative/special) Frobenius algebras, bimodules and bimodule homomorphisms is symmetric monoidal.
We can take the category of commutative or special Frobenius algebras and algebra homomorphisms as our new category of objects $\D_0$. Since this is a full subcategory, all structure functors, natural isomorphisms and companions and conjoints are still well-defined. It follows immediately that this defines a fibrant symmetric monoidal double category.
\end{rmk}


\begin{rmk}
The bicategory of dagger Frobenius algebras, dagger bimodules and bimodule homomorphisms is also symmetric monoidal.
To see this, we consider the double category where $\D_0$ is the category of dagger Frobenius algebras and self conjugate algebra homomorphisms. For $\D_1$ we take the category of dagger bimodules and extended bimodule homomorphisms $(f_1,f,f_2)$, of which $f_1$ and $f_2$ are self conjugate. These categories are symmetric monoidal subcategories of our original $\D_0$ and $\D_1$, since all structure natural isomorphisms for monoidal categoties are self-conjugate. 
Similarly, all structure functors and natural isomorphisms of symmetric monoidal double categories are well-defined.
Self conjugate algebra homomorphisms are exactly those morphisms of which the companion and conjoint are dagger bimodules; therefore, the double category is fibrant. 
\end{rmk}

\begin{eg}
When we take the category $Ab$ of Abelian groups and group homomorphisms as our symmetric monoidal category, we obtain the double category $\cMod$ of rings, ring homomorphisms, bimodules and bimodule homomorphisms. As a consequence of Lemmas ~\ref{thm:sym11-symbi}, ~\ref{lem:algdouble}, ~\ref{lem:algsymmon}, and ~\ref{lem:algfib}, the horizontal bicategory $\cMod$ of $\lMod$ is a symmetric monoidal.
\end{eg}
