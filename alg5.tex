...
We will consider the abstract notion of an {\bf algebra} $(A, \tinymult[white dot], \tinyunit[white dot])$ in a bicategory {\cat B}. This is a 0-cell $a$, together with a 1-cell $A \in {\cat B}(a,a)$ and a multiplication 2-cell 
$\tinymult[white dot]: A \hor A \rightarrow A$, as well as a unit 2-cell, $\tinyunit[white dot]: I_a \rightarrow A$, satisfying the associativity and unit law below. Here, we write $'\hor'$ for horizontal composition in the bicategory ${\cat B}$.
...
Let $A \in {\cat B}(a,a)$ and $B \in {\cat B}(b,b)$ be algebras. A {\bf monad map} $f: (A, \tinymult[white dot], \tinyunit[white dot]) \rightarrow (B, \tinymult[gray dot], \tinyunit[gray dot])$ is a pair $(f_1, f_2)$ where $f_1: a \rightarrow b$ is a 1-cell and $f_2$ is a 2-cell 
\begin{center}
\begin{tikzpicture}[scale=1.75]
\draw[fill=red, draw=black, opacity=0.5] (-1,0) -- (-1,-2) -- (0,-2) -- (0,0) -- (-1,0);  
\draw[fill=blue, draw=black, opacity=0.5] (0,0) -- (0,-2) -- (1,-2) -- (1,0) -- (0,0); 
      \draw (-.5,-2) to[in=-90, out=90] (-.1,-1);
      \draw (.1,-1) to[in=-90,out=90] (.5,0);   
      \node[morphism, minimum width=10mm] (l) at (0,-1) {$f_2$};
      \draw (0,0) to (l.north);
      \draw (l.south) to (0,-2) node[below] {};
      \node at (-.5,-2.2) {$A$};
      \node at (0,-2.2) {$f_1$};  
...
\begin{calign}\label{eq:monadmap}
\begin{pic}[xscale=1.25, yscale=1.5]
\draw[fill=red, draw=black, opacity=0.5] (-1,0) -- (-1,-2) -- (0,-2) -- (0,0) -- (-1,0);  
\draw[fill=blue, draw=black, opacity=0.5] (0,0) -- (0,-2) -- (1,-2) -- (1,0) -- (0,0); 
\node[white dot]  (ma) at (-.5,-1.5){}; 
      \draw (ma) to[in=-90, out=90] (-.1,-1);
      \draw (-.25,-2) to[in=0, out=90] (ma);
      \draw (-.75,-2) to[in=180, out=90] (ma);
      \draw (.1,-1) to[in=-90,out=90] (.5,0);   
      \node[morphism, minimum width=8mm] (l) at (0,-1) {$f_2$};
      \draw (0,0) to (l.north);
      \draw (l.south) to (0,-2) node[below] {};
     \node at (-.25,-2.2) {$A$};
          \node at (-.75,-2.2) {$A$};
...
\draw[fill=red, draw=black, opacity=0.5] (-1,0) -- (-1,-2) -- (0,-2) -- (0,0) -- (-1,0);  
\draw[fill=blue, draw=black, opacity=0.5] (0,0) -- (0,-2) -- (1,-2) -- (1,0) -- (0,0); 
\node[black dot]  (mb) at (.5,-.25){}; 
      \draw (.2,-1.25) to[in=-90, out=90] (.8,-.65);
      \draw (.8,-.65) to[in=0, out=90] (mb);
       \draw (.2,-.75) to[in=180, out=90] (mb);
      \draw (mb) to (.5,0);
      \draw (-.75,-2) to[in=-90, out=90] (-.2,-.75);
            \draw (-.25,-2) to[in=-90, out=90] (-.2, -1.25);
                        \node[morphism, minimum width=8mm] (l1) at (0,-1.25) {$f_2$};
      \node[morphism, minimum width=8mm] (l2) at (0,-.75) {$f_2$};
     \node at (-.25,-2.2) {$A$};
          \node at (-.75,-2.2) {$A$};
      \node at (0,-2.2) {$f_1$};  
      \node at (0,0.2) {$f_1$};  
...
Algebras and monad maps form a category, where composition of morphisms $f \ver g: (A, \tinymult[white dot], \tinyunit[white dot]) \xrightarrow{f} (B, \tinymult[gray dot], \tinyunit[gray dot]) \xrightarrow{g} (C, \tinymult[black dot], \tinyunit[black dot])$ is defined below. The identity morphism $\id_A$ is given by the coherence isomorphism $A \odot \id_a \rightarrow \id_a \odot A$. Note that by these definitions, we do not have strict associativity of the composition and strict identities. To fix this, we strictify horizontal composition of the maps $f_1$.

[Does this make sense?] 

\begin{center}
\begin{tikzpicture}[xscale=.75]
\draw[fill=red, draw=black, opacity=0.5] (-.2,3) -- (-.2,0) -- (3,0) to[in=-90,out=90] (2.1,1) -- (1.9,1) to[in=-90,out=90] (0,3) -- (-.2,3);
\draw[fill=purple, draw=black, opacity=0.5] (3,0) to[in=-90,out=90] (2.1,1) -- (1.9,1) to[in=-90,out=90] (0,3) -- (1,3) to[in=90,out=-90] (1.9,2) -- (2.2,2) to[in=90, out=-90] (4,0) -- (3,0);
\draw[fill=blue, draw=black, opacity=0.5] (1,3) to[in=90,out=-90] (1.9,2) -- (2.2,2) to[in=90, out=-90] (4,0) -- (4.2,0) -- (4.2, 3) -- (1,3);  
\draw (2,0) to  (2,3);
      \node[morphism, minimum width=5mm] (f) at (2,1) {$f_2$};
      \node[morphism, minimum width=5mm] (g) at (2,2) {$g_2$};
      \node at (2,-.2) {$A$};
      \node at (3,-.2) {$f_1$};  
             \node at (4,-.2) {$g_1$};  
       \node at (2,3.2) {$C$};   
...
The monoidal constraints $\alpha_{A,B,C}$ for algebras $(A, m_A, u_A) \in {\cat B}(a,a), (B, m_B, u_B) \in {\cat B}(b,b)$ and $(C, m_C, u_C) \in {\cat B}(c,c)$, respectively consist of the 1-cell component $\alpha_{a,b,c}$ in {\cat B} and the 2-cell component $\alpha_{A,B,C}$ in {\cat B} depicted below.
...
\draw[fill=blue, draw=black, opacity=0.5] (0,0) -- (0,-2) -- (1,-2) -- (1,0) -- (0,0); 
      \draw (-.5,-2) to[in=-90, out=90] (-.1,-1);
      \draw (.1,-1) to[in=-90,out=90] (.5,0);   
      \node[morphism, minimum width=10mm] (l) at (0,-1) {$\alpha_{A,B,C}$};
      \draw (0,0) to (l.north);
      \draw (l.south) to (0,-2) node[below] {};
      \node at (-.5,-2.1) {$(A \otimes B) \otimes C$};
      \node at (0,-2.25) {$\alpha_{a,b,c}$};  
             \node at (0,0.25) {$\alpha_{a,b,c}$};  
       \node at (.5,0.1) {$A \otimes (B \otimes C)$};   
        \node at (-.5,-.5) {$(a \otimes b) \otimes c$};    
             \node at (.5,-1.5) {$a \otimes (b \otimes c)$};
 \end{tikzpicture}
\end{center}

Equations~\ref{eq:monadmap} are satisfied by coherence of {\cat B} and naturality of the constraints. Likewise, the monoidal constraints $\lambda, \rho$ and $\sigma$ of ${\cat B}$ give rise to monoidal constraints on the category of algebras and monad maps.
...
\begin{equation}\label{eq:bimodule}
    ...
    \end{pic}
  \end{equation}
For readability, we sometimes write A-B-bimodules ${\bf M}$ as $_A{\bf M}_B$ and we abbreviate the following compositions: ${}_{\tinydot[white dot]}{\bf M} := {\bf M} \ver (\tinyunit[white dot] \tens \mathid)$, ${\bf M}_{\tinydot[gray dot]}:= {\bf M} \ver (\mathid \tens \tinyunit[gray dot])$, and $_{\tinydot[white dot]}{\bf M}_{\tinydot[gray dot]}:= {\bf M} \ver (\tinyunit[white dot] \tens \tinyunit[gray dot])$. 
Let $A \in {\cat B}(a,a),  B \in {\cat B}(b,b), C \in {\cat B}(c,c),  D \in {\cat B}(d,d)$ be algebras and let ${\bf M} \in {\cat B}(a,b), {\bf N} \in {\cat B}(c,d)$ be bimodules.  An {\bf extended bimodule homomorphism} ${\bf f}: {\bf M} \rightarrow {\bf N}$ is a triple $(f_l, f , f_r)$, where $f_l, f_r$ are monad maps and $f: M \hor f_{r1} \rightarrow f_{l1} \hor N$ is a 2-cell, such that the equation below holds.

\begin{equation}\label{eq:bimodule}
    \begin{pic}[xscale=1.5, yscale=5/3]
\draw[fill=red, draw=black, opacity=0.5] (0.8,0) -- (2,0) -- (2,2) -- (2,1.9) to[in=-90,out=90] (1,3) -- (0.8,3) -- (0.8,0);
\draw[fill=orange, draw=black, opacity=0.5] (2,2) -- (2,1.9) to[in=-90,out=90] (1,3) -- (2,3) -- (2,2);
    \draw (1.8,0) to (1.8,1);
    \draw[fill=blue, draw=black, opacity=0.5] (2,0)-- (3,0) to[in=-90, out=90] (2.1,2) -- (2,2) -- (2,0);
     \draw[fill=green, draw=black, opacity=0.5]  (3.2,0) --(3,0) to[in=-90, out=90] (2.1,2) -- (2,2) -- (2,3)-- (3.2,3) -- (3.2,0);   
    \draw (2,0) to (2,1);
    \draw (2.2,0) to (2.2,1);
\node[morphism, minimum width=10mm] (M) at (2,1) {$\mathbf{M}$};
\node[morphism, minimum width=5mm] (f) at (2,2) {$f_2$};
\draw (M) to (f);
\draw (f) to (2,3);
\node at (1.8,-.2) {$A$};
\node at (2,-.2) {$M$};
...
\node at (1.6, 2.7) {$a'$};
    ...
    =
    \begin{pic}
\draw[fill=red, draw=black, opacity=0.5] (.5,0) -- (2,0) -- (2,2) -- (1.9,2)to[in=-90, out=90] (1.6,3) -- (1.4,3) to[in=-90,out=90] (0.7,5) -- (.5,5) -- (.5,0); 
\draw[fill=orange, draw=black, opacity=0.5] (2,2) -- (1.9,2)to[in=-90, out=90] (1.6,3) -- (1.4,3) to[in=-90,out=90] (.7,5) -- (2,5) -- (2,2);
    \draw[fill=green, draw=black, opacity=0.5] (3.3,0) to[in=-90, out=90] (2.6,1) -- (2.4,1)  to[in=-90, out=90] (2.1,2) -- (2,2) -- (2,5) -- (3.5,5) -- (3.5,0) -- (3.3,0);
     \draw[fill=blue, draw=black, opacity=0.5]  (3.3,0) to[in=-90, out=90] (2.6,1) -- (2.4,1)  to[in=-90, out=90] (2.1,2) -- (2,2) -- (2,0) -- (3.3,0);  
     \draw (1.5,4) to (1.5,0);
     \draw (2.5,4) to (2.5,0);
     \node[morphism, minimum width=5mm] (fr) at (2.5,1) {$f_{r_2}$}; 
     \node[morphism, minimum width=5mm] (f) at (2,2) {$f$}; 
     \node[morphism, minimum width=12mm] (N) at (2,4) {$\mathbf{N}$};
          \node[morphism, minimum width=5mm] (fl) at (1.5,3) {$f_{l_2}$}; 
    \node at (1.1, .3) {$a$};
\node at (2.7,4.6) {$b'$};
\node at (2.7, 0.3) {$b$};
\node at (1.6, 4.6) {$a'$};
...
Composition of extended bimodule homomorphisms is defined as component wise compisition $(g_l, g ,g_r) \ver (f_l, f, f_r) = (g_l \ver f_l, g \ver f, g_r \ver f_r)$, where $g \ver f$ is defined as composition of monad maps. 
When $f_l$ and $f_r$ are both identities, this corresponds to the standard definition of a {\bf bimodule homomorphism}.
...
The associator, unitor, and swap constraints are given by the maps with the following components, where $\alpha, \lambda$, and $\rho$ are the monoidal constraints of {\bf B}. These are extended bimodule homomorphisms by coherence of {\bf B} and naturality of $\alpha, \lambda, \rho$ and $\sigma$. 
...
\begin{equation}\label{eq:coequaliser}
  \begin{pic}[xscale=4,yscale=1.66, thin]
    ...
    \draw[->] (tl) to node[left] {$(f * h * g)$} (bl);
    \draw[->] (t) to node[right] {$f *g$} (b);
...
Here, the composition $f*g$ is defined if $f$ is a 2-cell of the form $M \hor h \rightarrow f_{l1} \hor N$, and $g$ is a 2-cell of the form $K \hor g_{r1} \rightarrow h \hor L$ as follows:

\begin{equation}
  \begin{pic}[]
\draw[fill=red, draw=black, opacity=0.5] (-.2,3) -- (-.2,0) -- (1,0) -- (1,2) -- (0.9,2) to[in=-90, out=90] (0,3) -- (-.2,3);
\draw[fill=orange, draw=black, opacity=0.5] (1,2) -- (0.9,2) to[in=-90, out=90] (0,3) -- (1,3) -- (1,2);
    \draw[fill=blue, draw=black, opacity=0.5] (1,0) -- (2,0) -- (2,1) -- (1.9,1) to[in=-90, out=90] (1.1,2) -- (1,2) -- (1,0);
     \draw[fill=purple, draw=black, opacity=0.5]  (2,1) -- (1.9,1) to[in=-90, out=90] (1.1,2) -- (1,2) -- (1,3) -- (2,3) -- (2,1);
          \draw[fill=green, draw=black, opacity=0.5]  (2,0) -- (3,0) to[in=-90, out=90] (2.1,1) -- (2,1) -- (2,0); 
  \draw[fill=yellow, draw=black, opacity=0.5]  (3,0) to[in=-90, out=90] (2.1,1) -- (2,1) -- (2,3) -- (3.2,3) -- (3.2,0) -- (3,0); 
     \node[morphism, minimum width=5mm] (f) at (1,2) {$f$}; 
     \node[morphism, minimum width=5mm] (g) at (2,1) {$g$}; 
     \node at (.5,1.5) {$a$};
     \node at (1.5,0.75) {$b$};
     \node at (2.5, .3) {$c$};
          \node at (.5,2.7) {$a'$};
...
By commutativity of the left square, $c \ver (f * g)$ is another coequaliser of the upper two parallel maps, hence $f \tinydot[gray dot] g$ exists and is unique. 
...
It is left to verify functoriality of $\ominus$. When we unfold the definitions we see that we only need to check that $(h* k) \ver ( f * g) = (fh\ver f)*(k \ver gh)$, and $\mathid_{\bf M} * \mathid_{\bf M'} = \mathid_{\bf M \hor M'}$. Therefore, the diagrams below are equal, f we forget the middle part of the left diagram. As a consequence, the dashed arrows must be equal too. A similar argument shows that $\mathid_{\bf M} \tinydot[gray dot] \mathid_{\bf M'} = \mathid_{\bf M \tinydot[gray dot] M'}$.
...
%    \draw[->, dashed] (br) to [in=90, out=90] {$\hat{\xi}^{-1}$} (rd);
  \end{pic}  
   \begin{pic}[xscale=2.3, yscale=1.5, thin]
    ...
    \draw (tl) to  (-.3,0.25);
    \draw[->] (-.3,-0.25) to  (ld);
    \node at (-.3,0.125) {$(k \ver f) *(j \ver i)$};
    \node at (-.3,-.125) {$* (k \ver g)$};
    \draw (t) to  (1,0.3);
    \draw[->] (1,0.1) to  (md);
    \node at (1,0.2) {$(h \ver f) * (k \ver g)$};
...
\draw[->] ([yshift=-1.5pt] bl.east) to node[below] {$\mathid_{N_1} \hor {\bf N_2}_{\tinydot[gray dot]} \hor  {\bf N_3}_{\tinydot[gray dot]}$} ([yshift=-1.5pt] bm.west);
 ...
 \draw (tl) to (0,0.6) node[below]{$f_1 * f_B * f_2 * f_C * f_3$};
  \draw[->] (0,0.4) to (bl);
 \draw[->] (tm) to node[left] {$f_1 * f_2* f_3$} (bm); 
\end{tikzpicture}
...
It is left to prove that the diagrams given in points (iv),(v),(vi), and (ix) of Definition ~\ref{def:symmondoub} commute. Because $\fu$ and $\fx$ are globular, it suffices to verify commutativity of the diagrams only for the middle components of the extended module homomorphisms.
...
\draw[->] ([yshift=-1.5pt] bl.east) to node[below] {$\mathid_{B} \hor {U_B}_{\tinydot[gray dot]}$} ([yshift=-1.5pt] bm.west);
 ...
 \draw[->] (tl) to node[left] {$f * f * \mathid_B$} (bl);
 \draw[->] (tm) to node[left] {$f *\mathid_B$} (bm);
\end{tikzpicture}
