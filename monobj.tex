...
\fxnote[author=MS]{Also say something about how 1-strictness is not just for convenience, but in order to be able to write down the necessary composites of loose 2-cells; otherwise we'd need to insert constraints that are \textit{a priori} only tight 2-cells, hence fibrancy is needed already to even define things.}
...
\fxnote[author=MS]{Do all the lax cases first, then obtain the others by duality.}
...
\fxnote[author=MS]{Maybe we need a double annotation indicating the laxity direction of the 1-cells and the 2-cells separately?}
...
\begin{rmk}
  Note that even though $\fB$ is 1-strict, $\cMon_l\fB$ is not.\fxnote{Say a bit more here.}
\end{rmk}
